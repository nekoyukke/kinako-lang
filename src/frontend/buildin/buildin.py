...
def add_buildins(target: Context) -> None:
    ...
    target.general.types["ptr"] = type.PtrType()

    target.general.rights["owner"] = right.Right(right.AccessKind.WRITE, right.IdentityKind.UNIQUE)
    target.general.rights["observe"] = right.Right(right.AccessKind.READ, right.IdentityKind.UNIQUE)
    target.general.rights["view"] = right.Right(right.AccessKind.READ, right.IdentityKind.SHARED)
    target.general.rights["mut"] = right.Right(right.AccessKind.WRITE, right.IdentityKind.SHARED)
